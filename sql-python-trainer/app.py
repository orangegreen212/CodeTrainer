...
app = FastAPI(title="SQL & Python Interview Trainer v3.2")
...
async def export_tasks():
    ...
    # Header
    writer.writerow([
        'id', 'title_en', 'title_uk', 
        'what_to_find_en', 'what_to_find_uk',
    ...
    for p in problems:
        writer.writerow([
            ...
            p['title']['en'],
            p['title']['uk'],
            p.get('what_to_find', {}).get('en', ''),
            p.get('what_to_find', {}).get('uk', ''),
            p['description']['en'],
            p['description']['uk'],
            p['category'],
            p['difficulty'],
...
async def mistral_chat(request: Request):
    ...
    try:
        ...
        # UPDATED: Better prompt for code formatting
        system_prompt = f"""You are a helpful SQL/Python tutor. Help students with interview problems.

IMPORTANT FORMATTING RULES:
1. For SQL code: Use proper line breaks and indentation
2. For Python code: Use proper line breaks and indentation
3. Always format code in readable multi-line blocks
